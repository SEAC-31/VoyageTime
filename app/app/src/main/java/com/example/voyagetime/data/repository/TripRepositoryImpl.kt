...
class TripRepositoryImpl(
    ...
    private suspend fun ensureLocalUser(uid: String) {
        ...
        val fallbackEmail = "$uid@local.voyagetime"
        userDao.insertUserIfMissing(
            UserEntity(
                firebaseUid = uid,
                username = if (uid == LOCAL_USER_ID) "local_user" else "user_${uid.take(12)}",
                email = firebaseUser?.email ?: fallbackEmail
            )
        )
    }

    private fun currentUid(): String = authRepository.currentUserId() ?: LOCAL_USER_ID
